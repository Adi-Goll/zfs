...
function test_cleanup {
	log_must set_tunable64 ASYNC_BLOCK_MAX_BLOCKS 0xffffffffffffffff
...
function wait_until_extra_reserved {
	...
	free=$(zpool list | awk '{print $1,$4}' |
		...
		cut -d"." -f1)
	print "about to start the 2nd loop after first iteration"
	while (($free > 32)); do
		sleep 5
		free=$(zpool list | awk '{print $1,$4}' |
			grep $NESTEDPOOL | awk '{print $2}' | cut -d"M" -f1 |
...
log_must zfs set compression=off $FS0
log_must zfs set compression=off $FS1
...
#
log_must stride_dd -i /dev/zero -o $FS0FILE -b 512 -s 2 -c $NUMOVERWRITTENBLOCKS
...
#
print "before dd to fs1"
log_mustnot dd if=/dev/zero of=$FS1FILE bs=512 oflag=append conv=notrunc

print "after dd to fs1"
# for debugging purposes
log_must zpool list $NESTEDPOOL
...
for i in {1..100}; do
	...
	# properties in one zfs command and reduce
	...
	PROPERTIES=""
	for j in {1..100}; do
		PROPVAL=$(dd if=/dev/urandom bs=6000 count=1 | /usr/bin/base64 -w 0)
...
# every txg.
#
log_must set_tunable64 ASYNC_BLOCK_MAX_BLOCKS 10000
...
#
print "before with wait_until_extra_reserved"
wait_until_extra_reserved
print "done with wait_until_extra_reserved"
